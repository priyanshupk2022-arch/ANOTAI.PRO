...
import { getGlobalLimitsStatus } from "~/services/rateLimiter.server";
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const [errors, killSwitches, limits, failedActions, usageSummary, duplicates, pausedStores] = await Promise.all([
    getRecentErrors(undefined, 30),
    Promise.resolve(getGlobalKillSwitchStatus()),
    Promise.resolve(getGlobalLimitsStatus()),
    ...
    supabase.from("monthly_usage_counters").select("store_id, billing_month, ai_interactions_used, recovery_emails_sent, estimated_ai_cost")
      ...
      .limit(20),
    supabase.from("processed_webhooks").select("id", { count: "exact", head: true })
      .gte("processed_at", today.toISOString()),
    supabase.from("merchant_agent_settings").select("store_id", { count: "exact", head: true })
  ...
  return json({
    ...
    duplicatesCount: duplicates.count || 0,
    pausedStoresCount: pausedStores.count || 0,
    generatedAt: new Date().toISOString(),
  });
};

export default function AdminDebugPage() {
  const { errors, killSwitches, limits, failedActions, usageSummary, duplicatesCount, pausedStoresCount, generatedAt } = useLoaderData<typeof loader>();

  const switchColor = (active: boolean) => active ? '#EF4444' : '#10B981';
  const switchLabel = (active: boolean) => active ? '🔴 ACTIVE' : '🟢 OFF';
  ...
  return (
    <div style={{ fontFamily: 'monospace', padding: '40px', maxWidth: '1200px', margin: '0 auto', background: '#0F172A', color: '#E2E8F0', minHeight: '100vh' }}>
      <div style={{ marginBottom: '40px' }}>
        ...
        <p style={{ color: '#64748B', fontSize: '12px' }}>Generated: {new Date(generatedAt).toLocaleString()} · PHASE 10 READINESS</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', marginBottom: '40px' }}>
        ...
        <section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            {Object.entries(killSwitches).map(([key, value]) => (
              <div key={key} style={{ background: '#1E293B', padding: '12px', borderRadius: '8px', borderLeft: `4px solid ${switchColor(value as boolean)}` }}>
                <div style={{ fontSize: '10px', color: '#94A3B8', marginBottom: '2px' }}>{key}</div>
                <div style={{ fontSize: '12px', fontWeight: 700, color: switchColor(value as boolean) }}>{switchLabel(value as boolean)}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* Global Rate Limits */}
        <section>
          <h2 style={{ color: '#10B981', fontSize: '16px', fontWeight: 700, marginBottom: '16px' }}>⏱️ Real-time Throughput</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
             <div style={{ background: '#1E293B', padding: '12px', borderRadius: '8px' }}>
                <div style={{ fontSize: '10px', color: '#94A3B8' }}>AI Calls/min</div>
                <div style={{ fontSize: '18px', fontWeight: 800 }}>{limits.current.aiCallsThisMinute} / {limits.maxAiCallsPerMinute}</div>
             </div>
             <div style={{ background: '#1E293B', padding: '12px', borderRadius: '8px' }}>
                <div style={{ fontSize: '10px', color: '#94A3B8' }}>Emails/min</div>
                <div style={{ fontSize: '18px', fontWeight: 800 }}>{limits.current.emailsThisMinute} / {limits.maxEmailsPerMinute}</div>
             </div>
             <div style={{ background: '#1E293B', padding: '12px', borderRadius: '8px' }}>
                <div style={{ fontSize: '10px', color: '#94A3B8' }}>Webhooks Today</div>
                <div style={{ fontSize: '18px', fontWeight: 800 }}>{duplicatesCount}</div>
             </div>
             <div style={{ background: '#1E293B', padding: '12px', borderRadius: '8px' }}>
                <div style={{ fontSize: '10px', color: '#94A3B8' }}>Paused Stores</div>
                <div style={{ fontSize: '18px', fontWeight: 800, color: pausedStoresCount > 0 ? '#F59E0B' : '#E2E8F0' }}>{pausedStoresCount}</div>
             </div>
          </div>
        </section>
      </div>
            ...
            <tbody>
              {errors.map((err: any) => (
                <tr key={err.id} style={{ borderBottom: '1px solid #1E293B' }}>
                  ...
                  <td style={{ padding: '8px 12px', color: '#E2E8F0' }}>{err.error_message}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Usage Summary */}
      <section>
        <h2 style={{ color: '#10B981', fontSize: '16px', fontWeight: 700, marginBottom: '16px' }}>📊 Top 20 Stores by AI Usage (This Month)</h2>
        {usageSummary.length === 0 ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
            <thead>
              <tr style={{ background: '#1E293B' }}>
                {['Store ID', 'AI Interactions', 'Emails', 'Est. Cost'].map(h => (
                  <th key={h} style={{ padding: '8px 12px', textAlign: 'left', color: '#94A3B8' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {usageSummary.map((u: any) => (
                <tr key={u.store_id} style={{ borderBottom: '1px solid #1E293B' }}>
                  <td style={{ padding: '8px 12px', color: '#64748B' }}>{u.store_id}</td>
